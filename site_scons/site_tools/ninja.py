...
class NinjaState:
    ...
    def __init__(self, env, writer_class):
        ...
        self.variables = {
            "COPY": "cmd.exe /c copy" if env["platform"] == "windows" else "cp",
...
def generate(env):
    ...
    tools = env['TOOLS']
    if env["platform"] == "windows" and not "mingw" in tools:
    ...
    # where we expect it.
    env["IMPLICIT_COMMAND_DEPENDENCIES"] = True

    # Set build to no_exec, our subclass of FunctionAction will force
